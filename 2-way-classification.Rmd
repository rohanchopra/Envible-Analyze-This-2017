...
```
```{r}
card.data$mvar2 <- as.numeric(card.data$mvar2)
card.data$mvar2[card.data$mvar2 > 6] <- 6
card.data$mvar2 <- as.factor(card.data$mvar2)
str(card.data)
```
...
```{r}
    write.table(p,file = "Envible_VITVellore_28.csv",sep = ",",row.names = FALSE, col.names = FALSE)
```
